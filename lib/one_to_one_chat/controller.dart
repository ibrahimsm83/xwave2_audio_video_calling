...
class ChatController extends GetxController {
  ...
  ///Sender User
  final Rx<User_model> _senderUser = User_model(id: "",phoneNumber: "",avatar: "",username: "").obs;
  set senderUser(User_model senderUser) => _senderUser.value = senderUser;
  User_model get senderUser => _senderUser.value;

  ///Receiver User
  final Rx<User_model> _receiverUser = User_model(id: "",phoneNumber: "",avatar: "",username: "").obs;
  set receiverUser(User_model receiverUser) => _receiverUser.value = receiverUser;
...
print('-----set users----');
    ...
    //isLoading.value = true;
     EasyLoading.show();
    Response response=await apiService.getApiWithToken(fetch1To1chat+receiverId);
    print(response.body);
    EasyLoading.dismiss();
  ...
  sendTextMSg(String receiverId,User_model senderM, User_model receiverM)async{
...
print("listener is running -----");
   // chatList.add(ChatModel(content: contant, mediaType: mediaType, url: url, lati: lati, longi: longi, time: time, sender:senderUser , receiver: receiverUser));
    chatList.add(ChatModel(content: contant, mediaType: mediaType, url: url, lati: lati, longi: longi, time: time, sender: User_model(id:senderId,phoneNumber: '',avatar: '',username: ''), receiver: User_model(id:receiverId,phoneNumber: '',avatar: '',username: '')));
