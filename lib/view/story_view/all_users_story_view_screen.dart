...
class OpenUsersStoryView extends StatefulWidget {


  List<Statuses>? status;
  String? userName;
  String? userAvatar;
  OpenUsersStoryView({Key? key,this.status,this.userName,this.userAvatar}) : super(key: key);

  @override
  State<OpenUsersStoryView> createState() => _OpenUsersStoryViewState();
}

class _OpenUsersStoryViewState extends State<OpenUsersStoryView> {
  ...
  void setUserStatus(){
    for(int i=0;i<widget.status!.length;i++){
      if(widget.status![i].media!.isNotEmpty){
        if(widget.status![i].media!.first.type=="image"){
    ...
    sampleUsers=[
      UserModel(
        storyList,
       widget.userName??"",
      widget.userAvatar??dashboardController.user_model!.avatar,
        //ImageAssets.person2
      )
    ];
    setState(() {
